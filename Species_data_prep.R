...
hab_area <- read.csv("data/BBS_area_summRoutes.csv")

buffer_area <- read.csv("data/getAreaId_Summ_ROUTEBUFFER.csv")
...
missing_habitat <- join_test %>% 
  filter(is.na(route_buffer_area))
...
hab_full <- hab_full %>% 
  mutate(p_habitat = area/route_buffer_area) %>% 
  group_by(rt.uni) %>% 
  mutate(centered_p_habitat_x2 = 2*(p_habitat - mean(p_habitat)),
         centered_area_habitat = area - mean(area),
         mean_p_habitat = mean(p_habitat)) %>% 
  ungroup() %>% 
  rename(route = rt.uni) %>% 
  select(route,year,centered_p_habitat_x2,mean_p_habitat)
...
strat_mean_habitat <- new_data %>% 
  select(routeF,route,mean_p_habitat) %>% 
  distinct() %>% 
  arrange(routeF)

habitat_pred <- hab_full %>% 
  left_join(.,strat_mean_habitat,by = "route") %>% 
  select(routeF,year,centered_p_habitat_x2) %>% 
  pivot_wider(values_from = centered_p_habitat_x2,
              names_from = year,
              id_cols = routeF) %>% 
  arrange(routeF) 
...
stan_data[["c_habitat"]] <- new_data$centered_p_habitat_x2
...
stan_data[["route_habitat"]] <- as.numeric(strat_mean_habitat$mean_p_habitat - mean(strat_mean_habitat$mean_p_habitat))
